  ...
  return (
    <View style={styles.devicePickerContainer}>
      ...
      >
        ...
        >
          <View
            style={[styles.modalContent, { transform: [{ translateY: 0 }] }]}
          >
              ...
              >
                ...
                <View style={styles.deviceInfo}>
                  ...
                  <Text style={styles.deviceStats}>
                    {device.product_count} productos • Actualizado{" "}
                    {new Date(device.last_update).toLocaleDateString("es-ES")}
                  </Text>
                </View>
...
export default function ProductsScreen() {
  ...
  const [productsOFFSource, setProductsOFFSource] = useState<{
    [barcode: string]: "cache" | "api";
  }>({});
  ...
  const fetchAllProductsData = useCallback(
    async (productsList: Product[]) => {
      ...
      const batchSize = 3;
      const barcodesToProcess = Array.from(uniqueBarcodes).filter(
        (barcode) => barcode && !productsOFFData[barcode as string]
      );
  ...
  ); // Quitamos productsOFFData para evitar el bucle infinito
      ...
      <View style={styles.welcomeContainer}>
        <View style={styles.welcomeHeader}>
          ...
          <Text style={styles.welcomeSubtitle}>
            Organiza tu despensa de manera inteligente
          </Text>
        </View>
        ...
        <View style={styles.welcomeContent}>
          ...
          <View style={styles.stepsContainer}>
            ...
            <View style={styles.step}>
              ...
              <Text style={styles.stepText}>
                Vincula el armario escaneando su código QR
              </Text>
            </View>
            ...
            <View style={styles.step}>
              ...
              <Text style={styles.stepText}>
                ¡Empieza a organizar y monitorear tus productos!
              </Text>
            </View>
          </View>
          ...
          >
            <Ionicons
              ...
              size={24}
              color="white"
              style={styles.welcomeButtonIcon}
            />
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() => router.push("/ProductScanner")}
      >
      ...
      <ProductDetailsModal
        ...
        productData={
          productDetailsFromOFF
            ? {
                product: {
                  product_name:
                    productDetailsFromOFF.product_name || selectedProduct?.name,
                  brands: productDetailsFromOFF.brands || "",
                  image_url:
                    productDetailsFromOFF.image_url ||
                    selectedProduct?.imageUrl ||
                    "",
                  nutriscore_grade:
                    productDetailsFromOFF.nutriscore_grade || undefined,
                  ecoscore_grade:
                    productDetailsFromOFF.ecoscore_grade || undefined,
                  ingredients_text:
                    productDetailsFromOFF.ingredients_text || "",
                  additives_tags: productDetailsFromOFF.additives_tags || [], // Añadir esta línea para los aditivos
                  additives_original_tags:
                    productDetailsFromOFF.additives_original_tags || [], // Añadir también esta línea
                  labels_tags: productDetailsFromOFF.labels_tags || [],
                  nutriments: {
                    energy_100g:
                      productDetailsFromOFF.nutriments?.energy_100g || 0,
                    proteins_100g:
                      productDetailsFromOFF.nutriments?.proteins_100g || 0,
                    carbohydrates_100g:
                      productDetailsFromOFF.nutriments?.carbohydrates_100g || 0,
                    fat_100g: productDetailsFromOFF.nutriments?.fat_100g || 0,
                    fiber_100g:
                      productDetailsFromOFF.nutriments?.fiber_100g || 0,
                    salt_100g: productDetailsFromOFF.nutriments?.salt_100g || 0,
                    sugars_100g:
                      productDetailsFromOFF.nutriments?.sugars_100g || 0,
                    saturated_fat_100g:
                      productDetailsFromOFF.nutriments?.saturated_fat_100g || 0,
                    sodium_100g:
                      productDetailsFromOFF.nutriments?.sodium_100g || 0,
                    calcium_100g:
                      productDetailsFromOFF.nutriments?.calcium_100g || 0,
                    iron_100g: productDetailsFromOFF.nutriments?.iron_100g || 0,
                    trans_fat_100g:
                      productDetailsFromOFF.nutriments?.trans_fat_100g || 0,
                    cholesterol_100g:
                      productDetailsFromOFF.nutriments?.cholesterol_100g || 0,
                    vitamin_a_100g:
                      productDetailsFromOFF.nutriments?.vitamin_a_100g || 0,
                    vitamin_c_100g:
                      productDetailsFromOFF.nutriments?.vitamin_c_100g || 0,
                    vitamin_d_100g:
                      productDetailsFromOFF.nutriments?.vitamin_d_100g || 0,
                  },
                },
                status: 1,
                source: selectedProduct?.barcode
                  ? productsOFFSource[selectedProduct.barcode] || "api"
                  : "api",
              }
            : null
        }
